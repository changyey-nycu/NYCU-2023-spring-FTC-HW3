...
voting::Status localRegisterVoter(Voter _voter)
{
  ...
  if (exist == 0)
  {
    ...
    memcpy(pk, _voter.public_key().c_str(), crypto_sign_PUBLICKEYBYTES);
...
Status eVotingServer::CastVote(ServerContext *context, const Vote *vote, voting::Status *status)
{
  ...
  if (isSync == 0)
  {
    voting::ServerVote serverVote;
    ClientContext remotecontext;
    voting::Status remoteVstatus;
    serverVote.set_voter_name(voter_name);
    serverVote.set_election_name(election_name);
    serverVote.set_choice_name(choice_name);
    Status remoteStatus = GRPCconnect->remoteStub_->SyncVote(&remotecontext, serverVote, &remoteVstatus);
  }
...
void synRegisterVoter(std::string name)
{
  ...
  if (!status.ok())
  {
    std::cout << status.error_code() << " RPC failed" << std::endl;
  }
}
...
Status eVotingServer::SyncVote(ServerContext *context, const voting::ServerVote *serverVote, voting::Status *status)
{
  ...
  std::cout << "recv vote " << election_name << " " << choice_name << " by " << voter_name << std::endl;
  // check previous vote
  pqxx::connection c3(DBCONNECTINFO);
...
void controller()
{
  ...
  std::cout << "There is the primary server controller, please input the instruction to control the voting server\n"
            << instruction;
  // std::cout << "wait...\n";
  while (std::cin >> str)
  {
